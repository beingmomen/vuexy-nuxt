...
export const actions = {
    showDataFromApi({ dispatch }, payload) {
        dispatch("showData", payload.data)
    },
    editInDB({ state, dispatch }) {
        return this.$axios.$put(`/admins/update/${state.data.id}`, state.data).then(res => {
            if (res.success) {
                this.$router.push(this.localePath("/dashboard/admins"));
                dispatch("showSuccessMsg", this.app.i18n.t("msg.edit"), { root: true })
            }
        })
